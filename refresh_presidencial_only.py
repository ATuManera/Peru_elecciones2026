...
def current_targets(conn: sqlite3.Connection, limit: int | None) -> list[str]:
    sql = """
        SELECT codigo_mesa
...
def main() -> int:
    ap = build_argument_parser(description="Refresh incremental solo de Presidencial (idEleccion=10)")
    ...
    targets = current_targets(conn, args.limit)
    print(f"Mesas objetivo desde SQLite para {TARGET_NAME}: {len(targets)}")
    if not targets:
        conn.close()
        return 0
